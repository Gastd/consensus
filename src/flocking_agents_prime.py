...
# consensus_state = [v0, v1, v2, v3, theta0, theta1, theta2, theta3, x0, x1, x2, x3, y0, y1, y2, y3]
				...
				u1_sum_p = 0
				dtheta = 0
				dx = very_large_number
				dy = very_large_number
				for j in range(n):
					if(j != this_robot):
						v_error = consensus_state[this_robot, 0] - consensus_state[j, 0]
						dx = consensus_state[2*n + j, 0] - consensus_state[2*n + this_robot, 0]
						dy = consensus_state[3*n + j, 0] - consensus_state[3*n + this_robot, 0]
						dtheta = np.arctan2(dy, dx) - consensus_state[n + this_robot, 0]
						dtheta = np.arctan2(np.sin(dtheta), np.cos(dtheta))
						if dx == 0 or dy == 0:
							continue
						u1_sum_p += ((np.cos(dtheta) + 1)/2)**6/(np.sqrt(dx*dx + dy*dy))
						u1_sum += A[this_robot, j]*v_error

				u1 = -k * u1_sum - l * np.sign(u1_sum) - k_pos * u1_sum_p
				# u1 = -k * u1_sum - l * np.arctan(u1_sum*100)/np.pi*2 - k_pos * u1_sum_p
				# print('dtheta: ', dtheta)
				# print('dx: ', dx)
				# print('dy: ', dy)
				...
				# u2 = -k * u2_sum - l * np.arctan(u2_sum*100)/np.pi*2
